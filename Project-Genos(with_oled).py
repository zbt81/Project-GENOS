...
# C Pose
def pose_C():
    set_pose(70, 60, 30, 90, 0)  # Adjust degrees as needed for C
    update_oled("C")
...
def update_oled(letter):
    oled.fill(0)  # Clear the display
    print(f"Displaying: {letter}")  # Debugging step to check the letter being passed
    oled.text(f"Current letter: {letter}", 0, 0)  # Display current letter
    oled.show()  # Update OLED display
    time.sleep(0.5)  # Small delay to ensure the OLED refreshes
